...
`v<version>` tag. Git installations run the package's `prepare` script, which
generates the ts-proto sources in `src/gen` from `proto/` and compiles `dist`
for consumers; neither `src/gen` nor `dist` is ever committed.
